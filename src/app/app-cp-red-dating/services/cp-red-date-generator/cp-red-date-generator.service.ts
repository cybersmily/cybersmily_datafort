...
export class CpRedDateGeneratorService {
  ...
  private _cpDate: BehaviorSubject<CpRedDate> = new BehaviorSubject<CpRedDate>(
    new CpRedDate()
  );
  ...
  generate(settings?: any) {
    if (this._chart && this._roleLifepah) {
      this._cpDate.next(
        this.generateDate(this._chart, this._roleLifepah, settings)
      );
    } else {
      forkJoin({
        dateData: this.dataService.GetJson<{ cpred: CpRedDatingMainChart }>(
          CpRedAppFiles.DATING_CHARTS
        ),
        roleData: this.dataService.GetJson<{
          corebook: { roles: Array<CpRedLifepathCoreRoleChartEntry> };
  ...
  private generateDate(
    dateCharts: CpRedDatingMainChart,
    rolepathChart: Array<CpRedLifepathCoreRoleChartEntry>,
    settings?: any
  ): CpRedDate {
    ...
    redDate.activity = this.generateActivity(
      dateCharts.activity.find((act) => act.name === location.value).chart
    );
    ...
    if (redDate.type !== 'ghosted') {
      const dateTypeChart = dateCharts.datetypes.types.find(
        (type) => type.name === redDate.type
      );
      // beginning of the date
      redDate.beginning = this.generateEvent(
        dateTypeChart.beginning,
        redDate.keywords,
        dateCharts.keywords,
        dateCharts.variables,
        rolepathChart
      );
      // middle of the date
      redDate.middle = this.generateEvent(
        dateTypeChart.middle,
        redDate.keywords,
        dateCharts.keywords,
        dateCharts.variables,
        rolepathChart
      ...
      // end of the date
      redDate.end = this.generateEvent(
        dateTypeChart.end,
        redDate.keywords,
        dateCharts.keywords,
        dateCharts.variables,
  ...
  private generateLocation(
    chart: Array<KeyValue<string, string>>
  ...
  private generateEvent(
    ...
    keywordChart: Array<string>,
    variables: Array<NameChart>,
    roleChart: Array<CpRedLifepathCoreRoleChartEntry>
  ): string {
  ...
  private processChoices(event: string): string {
    ...
    const options = parseString.substring(
      0,
      parseString.indexOf(CHOICE_DELIMITER)
    );
    const value = this.dice.rollRandomItem(options.split('|'));
    return event.replace(
      `${CHOICE_DELIMITER}${options}${CHOICE_DELIMITER}`,
      value
    );
  }

  private processVariable(event: string, variables: Array<NameChart>): string {
    const parseString = event.substring(
      event.indexOf(VARIABLE_DELIMITER_START) + 2
    );
    const chartName = parseString.substring(
      0,
      parseString.indexOf(VARIABLE_DELIMITER_END)
    );
    const chart = variables.find(
      (variable) => variable.name === chartName
    )?.chart;
    if (chart) {
      const value = this.dice.rollRandomItem(chart);
      return event.replace(
        `${VARIABLE_DELIMITER_START}${chartName}${VARIABLE_DELIMITER_END}`,
        value
      );
    }
    return event
      .replace(VARIABLE_DELIMITER_START, ' ')
      .replace(VARIABLE_DELIMITER_END, ' ');
  }

  private processDice(event: string): string {
    const parseString = event.substring(
      event.indexOf(DICEROLLING_DELIMITER) + 2
    );
    const diceRoll = parseString.substring(
      0,
      parseString.indexOf(DICEROLLING_DELIMITER)
    );
    const results = Math.ceil(this.dice.rollMoreDice(diceRoll).total);
    return event.replace(
      `${DICEROLLING_DELIMITER}${diceRoll}${DICEROLLING_DELIMITER}`,
      results.toString()
    );
  }

  private processLifepath(
    event: string,
    roleChart: Array<CpRedLifepathCoreRoleChartEntry>
  ): string {

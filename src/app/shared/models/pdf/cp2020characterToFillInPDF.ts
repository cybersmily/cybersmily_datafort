...
export class Cp2020characterToPDF {
  ...
  private addStats(doc: jsPDF, stats: Cp2020StatBlock, line: number): number {
    ...
    line = line + 12;
    doc.setTextColor('black');
    this.createStatField(
      doc,
      'INT',
      stats.INT.Adjusted.toString(),
      this._left,
      line
    );
    this.createStatField(
  ...
  private printSkills(
    ...
    skills.forEach((s) => {
      ...
      doc.text(
        `[${s.value}|${s.value + stat}] ${s.chipped ? 'X' : ''}`,
        col + 38,
        line
      );
      line += 4;
  ...
  private createTextField(
    ...
    handleField.textAlign =
      align === 'center' ? 'center' : align === 'right' ? 'right' : 'left';
    doc.addField(handleField);
    doc.rect(x, y, width, height, 'S');
  }

  private createMultiLinField(
    doc: jsPDF,
    value: string,
    x: number,
    y: number,

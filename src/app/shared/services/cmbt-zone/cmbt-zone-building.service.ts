...
export class CmbtZoneBuildingService {
  ...
  createBuildings(num: number, dice: DiceService): Observable<Array<string>> {
    ...
    const data = this.dataService.GetJson<CmbtZoneData>(
      JsonDataFiles.CP2020_COMBAT_ZONE_DATA_JSON
    );
    const gangs = this.dataService.GetJson<Array<string>>(
      JsonDataFiles.CP2020_COMBAT_ZONE_GANGS_JSON
    );

    return forkJoin([data, gangs]).pipe(
      map((results) => {
  ...
  generateList(num: number, dice: DiceService): Array<string> {
    ...
    for (let i = 0; i < num; i++) {
      let building = dice.getRandomValue(
        this._buildingData.buildings.condition
      );
      building += ` ${dice.getRandomValue(this._buildingData.buildings.types)}`;
      let occupates = dice.getRandomValue(
        this._buildingData.buildings.occupates
      );
      if (occupates !== 'empty' && !building.startsWith('Burned')) {
        ...
        building += ` It is being used as a ${dice.getRandomValue(
          this._buildingData.buildings.use
        )}`;
      } else {
        building += `, empty.`;
      }

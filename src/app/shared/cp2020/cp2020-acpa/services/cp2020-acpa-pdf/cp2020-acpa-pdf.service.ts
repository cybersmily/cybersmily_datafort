...
export class Cp2020ACPAPdfService {
  ...
  saveFile(acpa: Cp2020ACPA) {
    ...
    let line = this.addTitle(doc, PdfPageSettings.MARGIN_TOP - 5, 'P O W E R   A R M O R   S P E C I F I C A T I O N S');
    line = this.addDetailsSection(doc, acpa, line);
    line = this.addTitle(doc, line, 'S   P   A   C   E   S');
    line = this.addSpaceSection(doc, acpa, line);
    line = this.addTitle(doc, line, 'E Q U I P M E N T   C A R R I E D / I N S T A L L E D');
  ...
  private addDetailsSection(doc: jsPDF, acpa: Cp2020ACPA, line: number): number {
    const start = line;
    const left = PdfPageSettings.MARGIN_LEFT + 3;
    doc.text(`SUIT NAME: ${acpa.name}`, left, line + 5);
    doc.text(`MANUFACTURER: ${acpa.manufacturer}`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`TOTAL WEIGHT: ${acpa.totalWeight.toLocaleString()}kg`, left, line + 5);
    doc.text(`TOTAL COST: ${acpa.totalCost.toLocaleString()}eb`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`CHASSIS TYPE: ${acpa.chassis.name} STR ${acpa.chassis.str}`, left, line + 5);
    doc.text(`CHASSIS WEIGHT: ${acpa.chassis.weight.toLocaleString()}kg`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`ARMOR: SP ${acpa.armor.sp}`, left, line + 5);
    doc.text(`CHASSIS CAPACITY/LIFT: ${acpa.chassis.carry.toLocaleString()}kg/${acpa.chassis.lift.toLocaleString()}kg`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`SIB/DFB: ${acpa.sib > 0 ? '+':''}${acpa.sib}/${acpa.sib > 0 ? '+':''}${acpa.dfb}`, left, line + 5);
    doc.text(`TROOPER SIZE: ${acpa.trooperSize.toLocaleString()}kg`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`PUNCH:${acpa.punch}    KICK:${acpa.kick}    CRUSH:${acpa.crush}`, left, line + 5);
    doc.text(`TOUGHNESS MODIFIER: ${acpa.chassis.toughness}`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM;
    doc.text(`REALITY INTERFACE: ${acpa.realityInterface.name}`, left, line + 5);
    doc.text(`CONTROL SYSTEM: ${acpa.controlSystem.name}`, PdfPageSettings.MIDPAGE, line + 5);
    line += PdfPageSettings.LINEHEIGHT_SM + 3;

    doc.rect(PdfPageSettings.MARGIN_LEFT, start, PdfPageSettings.MARGIN_RIGHT - PdfPageSettings.MARGIN_LEFT, line - start, 'S');
  ...
  private addSpaceSection(doc: jsPDF, acpa:Cp2020ACPA, line: number): number {
    ...
    const width = 33.3;
    line = this.addLocationSection(doc,'HEAD (1)', start, left, width, acpa.locations.head);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
    left += width;
    line = this.addLocationSection(doc,'R.ARM (2)', start, left, width, acpa.locations.rarm);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
    left += width;
    line = this.addLocationSection(doc,'L.ARM (3)', start, left, width, acpa.locations.larm);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
    left += width;
    line = this.addLocationSection(doc,'R.LEG (4-5)', start, left, width, acpa.locations.rleg);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
    left += width;
    line = this.addLocationSection(doc,'L.LEG (6-7)', start, left, width, acpa.locations.lleg);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
    left += width;
    line = this.addLocationSection(doc,'TOROS (9-0)', start, left, width, acpa.locations.torso);
    end = (line > end) ? line : end;
    doc.rect(left, start, width, end - start, 'S');
  ...
  private addLocationSection(doc: jsPDF, title: string, line: number, left: number,width: number, location: Cp2020ACPALocation): number {
    left += 1;
    ...
    location.internalComponents.forEach((item, i) => {
      ...
      let name = (item?.name.startsWith('(')) ? '  *see above*' : item?.name ?? '';
    ...
    location.internalComponents.forEach((item, i) => {
      ...
      let name = (item?.name.startsWith('(')) ? '  *see above*' : item?.name ?? '';
      doc.text(`${lead}${name}`, left, line + 5);
      doc.text(`${item?.sp ?? '-'}/${item?.sdp ?? '-'}`, leftMargin, line + 5, {align: 'right'});
      line += PdfPageSettings.LINEHEIGHT_XXS;
    });
    doc.setFontSize(9);
  ...
  private addCarriedSection(doc: jsPDF, acpa:Cp2020ACPA, line: number): number {
    const start = line;
    const columnIndex = Math.ceil(acpa.equipment.length/2);
    const newline = this.addCarriedColumn(doc, start, PdfPageSettings.MARGIN_LEFT,PdfPageSettings.MIDPAGE -10, acpa.equipment.slice(0, columnIndex) );
    line = this.addCarriedColumn(doc, start, PdfPageSettings.MIDPAGE, PdfPageSettings.MARGIN_RIGHT -10, acpa.equipment.slice(columnIndex) );
  ...
  private addCarriedColumn(doc: jsPDF, line: number, left: number, right: number, components: Array<Cp2020ACPAComponent|Cp2020ACPAWeapon>): number {
    const start = line;
    line += PdfPageSettings.LINEHEIGHT_SM;
    ...
    components.forEach(item => {
      ...
      doc.text(`${item?.sp ?? '-'}/${item?.sdp ?? '-'}/${item?.weight.toLocaleString() ?? '-'}kg`, right, line + 5, {align: 'right'});
      line += PdfPageSettings.LINEHEIGHT_SM;
    });
    line += PdfPageSettings.LINEHEIGHT_XXS;
    return line;

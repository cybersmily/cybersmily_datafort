...
export class Cp2020RolesDataService {
  ...
  getRoles(isIU?: boolean): Observable<Array<Cp2020Role>> {
    ...
    this._iu = isIU;
    const fileName = isIU
      ? JsonDataFiles.IU_ROLES_LIST_JSON
      : JsonDataFiles.CP2020_ROLES_LIST_JSON;
    return this.dataService
      .GetJson<{ roles: Array<Cp2020Role> }>(fileName)
      .pipe(
        map((data) => {
  ...
  rollRandomRole(
    dice: DiceService,
    isIU?: boolean
  ): Observable<Cp2020PlayerRole> {
    return this.getRoles(isIU).pipe(
      map((roles) => {

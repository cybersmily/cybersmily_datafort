import { BigLeagueContact } from '../../models/big-league-contact';
...
describe('FixerBigLeagueService', () => {
  ...
  it('should have Streeteal of 5', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      service.model.subscribe((data) => {
        expect(data).toBeTruthy(data);
        expect(data.contacts.length === 0).toBeTruthy(
          'Big League contacts should be empty.'
        );
        expect(data.streetdeal === 5).toBeTruthy(
          'Hot stuff streetdeal should be 5'
        );
        expect(service.totalPoints === 100).toBeTruthy(
          'Big League with Streedeal of 5 is not (5*2) * (5*2) = 100 resource points.'
        );
        expect(service.spentPoints === 0).toBeTruthy(
          'Big League spent points is not 0.'
        );
      });
    })
  ));

  it('should add Contact', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      ...
      service.model.subscribe((data) => {
        expect(data).toBeTruthy(data);
        expect(data.contacts.length === 4).toBeTruthy(
          'Big League contacts should have 4 elements.'
        );
        expect(service.spentPoints === 96.875).toBeTruthy(
          'Big League spent points is not 96.875.'
        );
      });
    })
  ));

  it('should not  add Contacts', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      ...
      service.model.subscribe((data) => {
        expect(data).toBeTruthy(data);
        expect(data.contacts.length === 0).toBeTruthy(
          'Big League contacts is not empty.'
        );
        expect(service.spentPoints === 0).toBeTruthy(
          'Big League spent points is not 0.'
        );
  ...
  it('should delete Contact', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      service.addContact(contacts.snitch); // 5 pts
      ...
      service.model.subscribe((data) => {
        expect(data).toBeTruthy(data);
        expect(data.contacts.length === 3).toBeTruthy(
          'Big League contacts should have 3 elements.'
        );
        expect(service.spentPoints === 46.25).toBeTruthy(
          'Big League spent points is not 46.25.'
        );
        expect(
          !data.contacts.some((c) => c.name.toLowerCase() === 'capable')
        ).toBeTruthy('Big League did not delete capable contact');
      });
    })
  ));

  it('should clear Contacts', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      service.addContact(contacts.snitch); // 5 pts
  ...
  it('should update Contacts', waitForAsync(
    inject([FixerBigLeagueService], (service: FixerBigLeagueService) => {
      service.reset();
      service.setStreetdeal(5);
      service.addContact(contacts.snitch); // 5 pts
      ...
      service.model.subscribe((data) => {
        expect(data.contacts.length === 2).toBeTruthy(
          'Big League contacts should be empty'
        );
        expect(service.spentPoints === 60).toBeTruthy(
          'Big League updated spent points is not 60.'
        );
        const update = data.contacts.find(
          (c) => c.name.toLowerCase() === 'snitch'
        );
        expect(update.cost === 20).toBeTruthy(
          'Big League udpated contact did not change cost'
        );
      });
    })
  ));

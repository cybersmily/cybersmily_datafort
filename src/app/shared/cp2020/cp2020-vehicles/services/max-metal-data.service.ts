...
export class MaxMetalDataService {
  ...
  loadMaxMetalData(): Observable<boolean> {
    return forkJoin([
      this.dataSerivce.GetJson<{ weapons: Array<VehicleWeapon> }>(
        JsonDataFiles.CP2020_MAXMETAL_WEAPONS_JSON
      ),
      this.dataSerivce.GetJson<{ options: any }>(
        JsonDataFiles.CP2020_MAXMETAL_OPTIONS_JSON
      ),
      this.dataSerivce.GetJson<{ mounts: any }>(
        JsonDataFiles.CP2020_MAXMETAL_MOUNTS_JSON
      ),
      this.dataSerivce.GetJson<{ baseTypes: any }>(
  ...
  loadWeapons(): Observable<Array<MaxMetalWeaponCategory>> {
    ...
    return this.loadMaxMetalData().pipe(
      map((data) => {
  ...
  private createOptionCategories(options: MaxMetalOption[]) {
    ...
    options.forEach((opt) => {
      if (!optionCategories[opt.type]) {
        optionCategories[opt.type] = {
          name: opt.type.replace(/_/g, ' '),
          id: opt.type,
          items: [],
        };
      }

...
export class Cp2020IuSkillConverterService {
   ...
   */
  convertCP2020SkillsToIU(
    skillList: Array<DataSkill>,
    characterSkils: Cp2020PlayerSkills
  ): Observable<Cp2020PlayerSkills> {
    return this.convertSkills(
      skillList,
      characterSkils,
      false,
      this.CP2020_MAP,
  ...
  private convertSkills(
    skillList: Array<DataSkill>,
    characterSkills: Cp2020PlayerSkills,
    hasOthers: boolean,
    source: string,
    target: string
  ) {
    return this.dataService
      .GetJson<Array<DataSkill>>(JsonDataFiles.IU_SKILLS_MAPPING_LIST_JSON)
      .pipe(
        map((data) => {
          characterSkills = this.mapSkills(
            characterSkills,
            data,
            source,
            target
  ...
  private mapSkills(
    ...
    mappings.forEach((m) => {
      const index = characterSkills.skills.findIndex((sk) => {
        return (
          sk.name.toLowerCase() === m[source].name.toLowerCase() &&
          sk.option === m[source].option
        );
      });
      if (index > -1) {

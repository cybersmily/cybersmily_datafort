...
export class Cp2020DatafortSvgBuilderService {
  ...
  private generateDatawalls(datawalls: Array<Coord>, gridSize: number): string {
    ...
    datawalls.forEach(datawall => {
      result += `<g><svg x="${datawall.x * gridSize}" y="${datawall.y * gridSize}" height="${gridSize}"
  ...
  private generateCodegates(codegates: Array<NrDatafortCodegate>, gridSize: number): string {
    let result = ``;
  ...
  private generateCPUs(cpus: Array<Coord>, gridSize: number): string {
    let result = ``;
    cpus.forEach(cpu => {
      result += `<g><svg x="${cpu.x * gridSize}" y="${cpu.y * gridSize}" height="${gridSize}"
          width="${gridSize}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
          ${NrNodeIcons.CPU}
        </svg></g>`;
    });;
  ...
  private generateRemotes(remotes: Array<NrDatafortRemote>, gridSize: number): string {
    let result = ``;
  ...
  private generateDefenses(defenses: Array<NrDatafortDefense>, gridSize: number): string {
    let result = ``;
  ...
  private createRect(x: number, y: number, height: number, width: number, fill: string, stroke: string, strokeWidth: number, fillOpacity: number): string {
  ...
  private createText(x: number, y: number, fontSize: number, fill: string, baseline: string, anchor: string, text: string): string {

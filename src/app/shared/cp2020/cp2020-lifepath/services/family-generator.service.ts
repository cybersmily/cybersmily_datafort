...
export class FamilyGeneratorService {
  ...
  private GetChartData(): Observable<FamilyChart> {
    if (!this.chartData) {
      return this.dataService
        .GetJson<FamilyChart>(JsonDataFiles.CP2020_LIFEPATH_FAMILY_JSON)
  ...
  CreateFamilyData(
    ...
    if (lifepathSource !== '') {
      family.familyRanking = this.GenerateRanking(
        chartData.familyRanking,
        lifepathSource
      );
      const isPack =
        family.familyRanking.toLowerCase().indexOf('nomad pack') > -1 ||
        family.familyRanking.toLowerCase().indexOf('pirate fleet') > -1;
      family.siblings = this.GenerateSiblings(
        isPack,
        lifepathSource,
        chartData
      );
      family.familyBackground = this.GenerateFamilyBackground(
  ...
  private GenerateFamilyBackground(
    ranking: string,
    source: string,
    chartData: FamilyChart
  ): string {
    ...
    familyDetail += this.GetParentsSomethingHappened(
      isSomething,
      source,
      chartData.parentSomethingHappen
    );

    // family standing/tragedy
    familyDetail += this.GetFamilyStanding(source, chartData.familyTragedy);
    // get childhood environment
    familyDetail += this.GetChildhoodEnvironment(
      source,
      chartData.childhoodEnvironment
    );
    return familyDetail;
  }
  ...
  private GetFamilyStanding(source: string, chart: any): string {
    ...
    if (this.diceService.generateNumber(1, 10) < tar) {
      ...
      const ok =
        source === 'NeoTribe'
          ? 'Family standing among your clan is good.'
          : 'Family status is OK.';
      return ' ' + ok;
    }
  }
   ...
   * @memberof LifepathFamilyComponent
   */
  private GetParentsSomethingHappened(
  ...
  private GetParentOptions(source): LifepahtParentOption {
    const results: LifepahtParentOption = {
      details: '',
    ...
    if (source === 'NeoTribe') {
      ...
      switch (this.diceService.generateNumber(1, 6)) {
        ...
        default:
          results.details +=
            'Something happened to both (or all) of your parents. ';
          results.somethingWrong = true;
      }
    } else {
  ...
  private GetNumberOfSiblings(isPack: boolean, source): number {
    ...
    let n =
      source === 'PacRimChina'
        ? this.diceService.generateNumber(1, 6)
  ...
  private GenerateSiblings(
    ...
    if (count > 0) {
      for (let i = 0; i < count; i++) {
        ...
        const isBro =
          source === 'PacRimChina'
            ? this.diceService.generateNumber(1, 6) % 3 === 0
            : this.diceService.generateNumber(1, 10) % 2 === 0;
        ...
        sibling.feeling = this.diceService.rollOnChart(
          chartData.siblingFeeling,
          source
        );

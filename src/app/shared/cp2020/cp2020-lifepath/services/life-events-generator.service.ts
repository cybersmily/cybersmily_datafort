...
export class LifeEventsGeneratorService {
  ...
  GenerateLifeEvents(
    ...
    years: string,
    isEventful: boolean
  ): Observable<LifepathEventsList> {
    return this.GetChartData().pipe(
      flatMap((data) =>
        of(this.GetEventList(years, data, lifepathSource, isEventful))
      )
    );
  }
  ...
  private GetChartData(): Observable<LifepathEventsCharts> {
    if (!this.chartData) {
      return this.dataService
        .GetJson<LifepathEventsCharts>(
          JsonDataFiles.CP2020_LIFEPATH_EVENTS_JSON
        )
  ...
  private GetEventType(
    ...
    const source = chartData.ChartRoll[lifepathSource]
      ? lifepathSource
      : 'CP2020';
  ...
  private GetDisaster(chart: LifepathEventsCharts, version: string) {
    let c =
      'Big Lose: ' + this.dice.rollOnChart(chart.Disasters.Event, version);
    c +=
      ' What are you going to do: ' +
      this.dice.rollOnChart(chart.Disasters.Response, version);
    return c;
  }
  ...
  private GetFriend(chart: LifepathEventsCharts, version: string) {
    ...
    let s =
      'Made a Friend: ' +
      (isMale ? "He's " : "She's ") +
      this.dice.rollOnChart(chart.Friends, version);
    if (s.indexOf('{0}') > -1) {
  ...
  private GetEnemy(chart: LifepathEventsCharts, version: string) {
    ...
    s +=
      ' Emnity started due to: ' +
      this.dice.rollOnChart(chart.Enemies.cause, version);
    s += ' ' + this.dice.rollOnChart(chart.Enemies.fracked, version);
    s +=
      ' Next time you meet, reaction is: ' +
      this.dice.rollOnChart(chart.Enemies.reaction, version);
    s +=
      ' Their backup is ' +
      this.dice.rollOnChart(chart.Enemies.resource, version);
    if (!isMale && s.indexOf('his') > -1) {
      s = s.replace(/his/gi, 'her');
    }
    if (!isMale && s.indexOf('him') > -1) {
      s = s.replace(/him/gi, 'her');
    }
    if (!isMale && s.indexOf('him') > -1) {
      s = s.replace("he's", "she's");
    }
    return s;
  }
  ...
  private GetRomance(chart: LifepathEventsCharts, version: string) {
    ...
    if (s.indexOf('Tragic') > -1) {
      s +=
        ' ' + this.dice.rollOnChart(chart.RomanticInvolvement.Tragic, version);
    } else {
      s +=
        ' ' +
        this.dice.rollOnChart(chart.RomanticInvolvement.Problems, version);
    }
    s +=
      ' ' + this.dice.rollOnChart(chart.RomanticInvolvement.Feeling, version);
    return s;
  }
}

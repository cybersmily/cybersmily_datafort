...
export class ArmorGeneratorService {
  ...
  generate(
    settings: CP2020ArmorRandomSettings,
    dice: DiceService,
    ...
    do {
      ...
      if (settings?.quality !== '') {
        armor.quality = clothingLists.qualities.filter(
          (q) => q.name === settings?.quality
        )[0];
      } else {
        armor.quality = dice.rollRandomItem<ArmorOption>(
          clothingLists.qualities
        );
      }

      if (settings?.style !== '') {
        armor.style = clothingLists.styles.filter(
          (q) => q.name === settings?.style
        )[0];
      } else {
      ...
      if (canBeArmor && armor.clothes.loc !== '') {
        ...
        if (armor.clothes.loc.includes('arm')) {
          armor.locations['rarm'] = armor.baseSP;
          armor.locations['larm'] = armor.baseSP;
        }
        if (armor.clothes.loc.includes('leg')) {
      ...
      if (
        typeof armor.clothes?.leather !== 'undefined' &&
        armor.cost < settings?.maxCost
      ) {
        armor.isLeather = settings.isLeather
          ? true
          : dice.generateNumber(0, 10) < 3;
      }
      armor.cost = this.armorCostCalculator.calculateCost(
        armor,
        clothingLists.armorChart
      );

...
export class CpRedWeatherGeneratorService {
  ...
  private _weatherForecast  = new BehaviorSubject<Array<CpRedDayWeather>>(new Array<CpRedDayWeather>());
  ...
  generate(system: string, season: string, timeRange: number) {
    if(this._charts) {
      this._weatherForecast.next(this.generateWeather(system, season, timeRange));
    } else {
    this.dataService
  ...
  private generateWeather(system: string, season: string, timeRange: number): Array<CpRedDayWeather>  {
    const selectedChart = this._charts.find(chart => chart.system === system);
    return this.rollWeather(selectedChart, season, timeRange);
  }

  private rollWeather(chart: CpRedWeatherChart, season: string, timeRange: number): Array<CpRedDayWeather> {
    ...
    for(let i = 0; i < timeRange; i++) {
      ...
      currCondition = (result.duration > 0) ? {condition: result.weather.condition, duration: result.duration} : null;
      results.push(result.weather);
    }
    return results;
  }

  generateDayWeather(chart: CpRedWeatherChart, season: string, continued?: {condition:string, duration: number}): {weather: CpRedDayWeather, duration: number} {
    let condition = continued?. condition ?? this.dice.rollRandomItem(chart[season].conditions);
    ...
    if(condition.toLowerCase() === 'strange') {
      ...
      if(strange.condition.includes(`${COLD_SNAP}/${HEAT_WAVE}`)) {
        if (temp < 60 || (season === WEATHER_SEASONS.WINTER.valueOf() && temp === 60)) {
    ...
    return {weather: {tempature: temp, condition: condition}, duration: weatherDuration - 1};
  }

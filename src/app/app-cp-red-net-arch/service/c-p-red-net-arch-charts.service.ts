...
export class CPRedNetArchChartsService {
  ...
  get charts(): Observable<CPRedArchCharts> {
    if (this._architect) {
      ...
      return this.dataService
        .GetJson<CPRedArchCharts>(JsonDataFiles.CPRED_NET_ARCH_CHARTS_JSON)
        .pipe(
          map((data) => {
  ...
  getProgram(name: string): Observable<NetArchProgram> {
    return this.programs.pipe(
      map(
        (data) =>
          data.find(
            (prog) => prog.name.toLocaleLowerCase() === name.toLocaleLowerCase()
          )[0]
      )
    );
  }
  ...
  getDemon(name: string): Observable<CPRedDemon> {
    return this.demons.pipe(
      map(
        (data) =>
          data.find(
            (demon) =>
              demon.name.toLocaleLowerCase() === name.toLocaleLowerCase()
          )[0]
      )
    );
  }

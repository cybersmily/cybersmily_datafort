import {
  ...
  NightMarketListing,
  NightMarketCategory,
} from '../../shared/cpred/c-p-red-economy/models';
import { of } from 'rxjs';
  ...
  describe('generate()', () => {
    it('should generate two categories', () => {
      ...
      component.itemList$.subscribe((data) => {
        ...
        expect(component['_itemList'][0]?.items.length).toBeGreaterThanOrEqual(
          1
        );
        expect(component['_itemList'][1]?.items.length).toBeGreaterThanOrEqual(
          1
        );
      });
    });
